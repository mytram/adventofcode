...
solver = Day04Solver.new(lines)

patrol_logs = solver.patrol_logs

assert(patrol_logs[0].guard_id, 10)
assert(patrol_logs[0].action, :begin)

assert(patrol_logs.last.guard_id, 99)
assert(patrol_logs.last.action, :wakeup)

patterns = solver.sleep_patterns

assert_truthy patterns['1518-11-01'][10]
assert_truthy patterns['1518-11-02'][99]
assert_truthy patterns['1518-11-03'][10]
assert_truthy patterns['1518-11-04'][99]
assert_truthy patterns['1518-11-05'][99]

assert_truthy patterns['1518-11-03'][10] == (24..28).to_a
assert_truthy patterns['1518-11-05'][99] == (45..54).to_a

assert solver.sleep_counts, 10 => 50, 99 => 30

assert solver.lazist_guard, [10, 50]
assert solver.most_likely_sleep_minute(10), [24, 2]

# puts solver.guard_minute_counts(10).inspect
# puts solver.guard_minute_counts(99).inspect

assert solver.call, 240
assert solver.call2, 4455

lines = DATA.each_line.map(&:strip).reject(&:empty?)

solver = Day04Solver.new(lines)

assert solver.call, 26281

assert solver.call2, 73001
